...
# Description
 ...
 - Use temporary datastructures to store the pending changes that a client has made to their user settings config
    - Changes will be flushed to the actual ones that the app uses when the client saves their changes through the "Save changes" button
 - When the client has changed at least one setting on any page (even if they change it back):
    - Display a new button with text "Save changes" with the same button constants as the "Back" button, but copied by value
    - Its location will always be the leftmost in the row of buttons at the bottom of the Settings page next to the "X" button
    - User settings will **only** be saved if the user clicks this button
    - Display a loading in place of the button text until the saving to **both** the in-memory datastructures representation of the user settings and their backing persistent SQLite DB are done
       - Once they are done saving:
          - The "Save changes" button will be hidden again until another setting changes
    - If the user interacts with the "X" button before saving changes:
       - If USER_SETTING_SCHEMA_UPDATE_HAS_ERRORED is true:
          - Do not save anything, just close the user settings page
       - Else:
          - Don't close the user settings page
          - Display a negative important alert with the text "You have unsaved changes" with 2 buttons, one has the accent color with text "Exit anyways", the other has the background color with text "Don't exit"
          - If the user interacts with "Exit anyways":
             - The page closes and nothing is written to the in-memory user settings datastructures or the persistent SQLite DB containing the user settings
          - If the user interacts with "Don't exit":
             - The alert closes and user remains on settings page
    - If USER_SETTING_SCHEMA_UPDATE_HAS_ERRORED is true:
       - Don't do anything
       - Display a negative important alert with the text "Due to a settings data migration failure, saving is currently disabled" with the button text "Ok"
 - Types of settings:
    ...
    - Subsettings page button with a ">" symbol at the right slider
       - Will open a new settings page with the subheader equivalent to the button's name 
          - Can contain all the setting types that a normal settings page will have
       - The header will show "Settings > <current-settings-page-header>"
 ...
 - There will be a margin at the bottom of the settings and subsettings pages to allow for the "X" button for exiting the settings pages to be displayed without covering up any settings options 
 ...
 - DO NOT IMPLEMENT THIS NOW, THIS IS JUST A PROPOSAL:
    - One day, there will be an option to upload your settings into the remote DB to sync up settings between multiple devices
# Settings
